...
数据采集时间：**2026-06-12**。Skill 入口层（M4）的单一 CLI + 子命令形态需要参数解析、子命令路由与自描述帮助；候选范围限定在轻量、ESM 友好、可被打包器（M8 定档 esbuild）打进单文件的方案。
...
- openapi-fetch、citty 仍为 0.x，升级需钉版本并关注 changelog（citty 0.1→0.2 即有 ESM-only、解析语义等真破坏性变更）；分发态打包器 esbuild 虽已 1.0 前的稳定线，作为 build-time 工具同样钉死版本以保产物可复现。
