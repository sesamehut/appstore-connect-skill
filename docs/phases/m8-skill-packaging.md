...
- **marketplace 仓库**位于 `C:\Source\sesamehut\plugins-marketplace`，清单为 `.claude-plugin/marketplace.json`。清单顶层 `name` 为 **`sesamehut-plugins`**、`owner` 为 "Sesame Hut Studio"。每个 `plugins[]` 条目形态为 `{ name, description, version, category, source:{ source:"url", url:<github .git>, ref:"main" } }`，且 sonara 条目的 `source.url` 实指 **`https://github.com/sesamehut/sonara-plugin.git`**。已见类目：`content-ops`（sonara）、`auth`（auth-client）。**注册是手动编辑该数组 + push**；version 手工同步（无自动化）；用户经 `/plugin marketplace update` 刷新。
